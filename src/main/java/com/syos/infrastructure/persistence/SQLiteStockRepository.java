...
public class SQLiteStockRepository implements StockRepository {
    ...
    public void save(StockBatch batch) {
        String sql = "INSERT OR REPLACE INTO stock_batches (batch_number, product_id, quantity, expiry_date, received_date) " +
                     "VALUES (?, ?, ?, ?, ?)";
        ...
        try (Connection conn = databaseManager.getConnection();
            ...
            pstmt.setString(4, batch.getExpiryDate().toString());
            pstmt.setString(5, batch.getReceivedDate().toString());
            pstmt.executeUpdate();
